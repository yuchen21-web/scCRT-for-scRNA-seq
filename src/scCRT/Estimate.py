...
class Estimate_time():
    def __init__(self, data, cell_labels, start_node=0, k=5, k_p=4, end_nodes=None, debug_level=None, norm=False):
        ...
        self.norm = norm # 在计算最小生成树是是否norm
    ...
    def get_lineages(self):
        self.clusters_links, self.clusters_links_norm = getInnerClusterKNN(self.cell_labels, self.data, k=self.k)
        if self.norm:
            links = self.clusters_links_norm
        else:
            links = self.clusters_links
        self.Lineage_class, self.branch_clusters = getLineage(links, self.cell_labels, start_node=self.start_node)
    ...
    def construct_initial_curves(self):
        ...
        for l_idx, lineage in enumerate(self.Lineage_class.lineages):
            ...
            #     np.array([self.cell_belong_type == k for k in lineage.lineage_index]))
            cell_mask = np.logical_or.reduce(
